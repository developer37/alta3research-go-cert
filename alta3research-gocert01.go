package main
// This project is to demonstrate proficiency in the GO language. A REST client written in Go will make a HTTP GET call to a NASA REST API to retrieve the current NEO's 
// (Near Earth Objects) Ids and urls which would be used to retrieve more details from the NASA site. The NEO ids and urls will be collected in a map and printed. 
// A follow up HTTP GET call will be made for an NEO and its details will be printed. 
// The code can be easily modified to print more pertinent data as well as re-packaged so that func GetNeoDetails can be  called by other Go programs to retrieve 
// more details for an NEO from the NASA REST API.
